...
from ner_model.model.process.ckiplab_models.driver import (
    CkipNerChunker,
    CkipPosTagger,
    CkipWordSegmenter,
)
...
class CkipTransformerHandler:
    ...
    def load_model(
        ...
        if not (os.path.exists(os.path.join(root_dir, model_type)) or os.path.exists(model_path)):
            logger.error(
                "Model file for '%s' type '%s' does not exist in %s",
                model_name,
                model_type,
                root_dir,
            )
        ...
        if model_type in ["ws", "pos", "ner"]:
            driver = CkipTransformerHandler._load_chinese_model(
                model_name, model_type, device_type, root_dir
            )
        elif model_type in ["eng_ner", "eng_pos"]:
            driver = CkipTransformerHandler._load_english_model(
                model_name, model_type, device_type, batch_size, model_path, root_dir
            )
        elif model_name == "translate" and model_type == "translate":
            driver = CkipTransformerHandler._load_translation_model(
                model_name, device_type, batch_size, model_path, root_dir
            )
        else:
    ...
    def _load_translation_model(
        model_name: str, device_type: int, batch_size: int, model_path: str, root_dir: str
    ):
