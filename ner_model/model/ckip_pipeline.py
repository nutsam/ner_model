...
class CkipTransformerPipeline:
    ...
    """
    def __init__(self, device_ch, device_eng_ner, device_eng_pos, device_translate,
                 batch_size_ch, batch_size_en,
                 model_name, eng_ner_model, eng_pos_model, translate_model):
        ...
        self.bert_model = model_name if model_name in ['bert_tiny', 'bert_base', 'albert_base'] else 'bert_tiny'
    ...
    def _download_and_load_models(self):
        ...
        # Load Chinese models
        self.ws_model = CkipTransformerHandler.load_model(self.bert_model, 'ws', self.device_ch, self.batch_size_ch)
        self.pos_model = CkipTransformerHandler.load_model(self.bert_model, 'pos', self.device_ch, self.batch_size_ch)
        self.ner_model = CkipTransformerHandler.load_model(self.model_name, 'ner', self.device_ch, self.batch_size_ch)
        # Load English models
        self.eng_ner_driver = CkipTransformerHandler.load_model(self.eng_ner_model, 'eng_ner', self.device_eng_ner, self.batch_size_en)
        self.eng_pos_driver = CkipTransformerHandler.load_model(self.eng_pos_model, 'eng_pos', self.device_eng_pos, self.batch_size_en)
    ...
    def _classify_text(self, text):
        ...
        mix_sentences = [s for s in sentences if not self._is_english_sentence(s) and re.findall(r"[A-Za-z0-9]{2,}", s)]
    ...
    def _predict_english(self, sentences, indices):
        ...
        for i, sent in enumerate(sentences):
            orig_idx = indices[i]
            temp_ws = [pos['word'].strip().upper() for pos in pos_preds[i]
                       if pos['word'].strip() and pos['word'].strip().upper() in sent.text.upper()]
            temp_pos = [pos['entity_group'] for pos in pos_preds[i]
                        if pos['word'].strip() and pos['word'].strip().upper() in sent.text.upper()]
            temp_ner = {}
    ...
    def _process_chinese(self, texts, max_length, use_delimiter, show_progress):
        """呼叫中文模型並過濾英文部份"""
        ws = self.ws_model(texts, batch_size=self.batch_size_ch, max_length=max_length,
                           use_delim=use_delimiter, show_progress=show_progress)
        pos = self.pos_model(ws, batch_size=self.batch_size_ch, max_length=max_length,
                             use_delim=False, show_progress=show_progress)
        ner = self.ner_model(texts, model_name=self.model_name, batch_size=self.batch_size_ch,
                             max_length=max_length, use_delim=use_delimiter, show_progress=show_progress)
    ...
    def _filter_chinese(self, ws_list, pos_list, ner_list):
        ...
        filtered_ws = [[token for token in tokens if not is_english(token)]
                       for tokens in ws_list]
        filtered_pos = [[pos for token, pos in zip(tokens, pos_list[i]) if not is_english(token)]
    ...
    def run_pipeline(self, texts, max_length, use_delimiter, show_progress):
        cleaned_texts, eng_inputs, eng_indices, mix_inputs, mix_indices = self._prepare_model_inputs(texts)
        ...
        ch_ws, ch_pos, ch_ner = self._process_chinese(cleaned_texts, max_length, use_delimiter, show_progress)
    ...
    def _extract_entities(self, ws_tokens, pos_tokens, ner_tokens, max_len=35):
        ...
        for token, pos in zip(ws_tokens, pos_tokens):
            ...
            if len(token) > 1 and len(pos) <= 4:
                if 'A' in pos: entities['adj'].add(token)
                if 'D' in pos: entities['adv'].add(token)
                if 'V' in pos: entities['v'].add(token)
                if pos in ['Na', 'Nb', 'Nc', 'PRON']: entities['n'].add(token)
        for ent in ner_tokens:
            word = ent.word.strip().upper()
            label = ent.ner
            valid = ('BIO' in self.model_name and label in {'PER', 'LOC', 'ORG'}) or \
                    (label in {'PERSON', 'ORG', 'EVENT', 'FAC', 'PRODUCT', 'GPE', 'LOC', 'NORP', 'WORK_OF_ART'})
            if valid and word:
    ...
    def process_output(self, original_texts, eng_ws, eng_pos, eng_ner, ch_ws, ch_pos, ch_ner, cleaned_texts):
        """整合英文與中文的實體結果"""
        results = {}
    ...
    def enhanced_cleaning(self, ner_results, original_texts):
        ...
        def should_keep(token):
            ...
            if ' ' in token and len(token) < 5:
                return False
            if has_chinese and any(ch.isalpha() for ch in token) and any(ch.isdigit() for ch in token) \
               and ' ' in token and len(token) < 8:
                return False
            return True
        ...
        for idx, entities in ner_results.items():
            cleaned[idx] = {label: {normalize(word) for word in words if should_keep(word)}
                            for label, words in entities.items() if words}
        return cleaned
    ...
    def get_named_entities(self, texts, use_batch, max_length, use_delimiter, show_progress):
        if use_batch:
            ...
            raw_results = self.process_output(texts, eng_ws, eng_pos, eng_ner, ch_ws, ch_pos, ch_ner, cleaned_texts)
        else:
            raw_results = {}
            for idx, line in enumerate(texts):
                eng_ws, eng_pos, eng_ner, ch_ws, ch_pos, ch_ner, cleaned_texts = self.run_pipeline(
                    [line], max_length, use_delimiter, show_progress)
                raw_results[idx] = self.process_output([line], eng_ws, eng_pos, eng_ner, ch_ws, ch_pos, ch_ner, cleaned_texts)
